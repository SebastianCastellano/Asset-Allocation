...
KSel=8; % numero di titoli da selezionare
fee=0.003; % pct fee
...
for i=1:NOff
    ...
    if offUnfit<threshUnfit
        ...
        %trova individuo con fit massimo tra quelli con unfit e fit maggiore.
        idx(unfit>=offUnfit)=(fit(unfit>=offUnfit)==max(fit(unfit>=offUnfit)));
        if all(~idx)
            %trova individuo con fit massimo tra quelli con unfit minore e
            %fit maggiore
